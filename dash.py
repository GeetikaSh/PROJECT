...
Original file is located at
    https://colab.research.google.com/drive/1CfRhzUCqkq_5v53jut9a18UyXJkpv0l8
"""
...
PROJECT/d3.py /
@GeetikaSh
GeetikaSh Update d3.py
Latest commit 6042aec 2 hours ago
 History
 1 contributor
350 lines (287 sloc)  12.5 KB
...
if select=='Model and Accuracy':
  ...
  if model=="ARIMA: log":
    ...
    train_data, test_data = df[3:int(len(df)*0.9)], df[int(len(df)*0.9):]
    model = sm.tsa.statespace.SARIMAX(df,order=(1,1,0),seasonal_order=(1,1,0,12))  
    fitted = model.fit()  
    fc=fitted.forecast(101, alpha=0.05)
    fc.index=test_data.index
    conf = forecast.conf_int(alpha=0.05)
